...
def _has_full_pysimplegui():
    if sg is None:
        return False
    required = ("theme", "Window", "Text", "InputText", "Button", "WIN_CLOSED")
    return all(hasattr(sg, attr) for attr in required)


""" GUI for loading a single file for Huganir lab synapse tracking """
def single_file_GUI():
    if not _has_full_pysimplegui():
        root = tkinter.Tk()
        root.withdraw()
        filenames = []
        initial_dir = "./"
        while True:
            filename = filedialog.askopenfilename(parent=root, initialdir=initial_dir, title="Please select input file")
            if not filename:
                print("Exiting analysis")
                exit()
            filenames.append(filename)
            initial_dir = os.path.dirname(filename) or "./"
            if not messagebox.askyesno("Input file", "Select another file?"):
                break
        save_path = filedialog.askdirectory(parent=root, initialdir=initial_dir, title="Please select save directory")
        if not save_path:
            print("Exiting analysis")
            exit()
...
def XTC_track_GUI(default_XY='0.83', default_Z='3'):
    if not _has_full_pysimplegui():
        root = tkinter.Tk()
        root.withdraw()
        XY_res = simpledialog.askstring("XTC and tracking input", "XY resolution (um/pixel)", initialvalue=str(default_XY), parent=root)
        Z_res = simpledialog.askstring("XTC and tracking input", "Z resolution (um/pixel)", initialvalue=str(default_Z), parent=root)
        ...
        initial_dir = "./"
        while True:
            input_path = filedialog.askdirectory(parent=root, initialdir=initial_dir, title="Please select input directory")
            if not input_path:
                print("Exiting analysis")
                exit()
            input_path = input_path + "/"
            ...
            initial_dir = input_path
            if not messagebox.askyesno("XTC and tracking input", "Select another folder?"):
                break
