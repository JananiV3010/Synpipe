...
parser = argparse.ArgumentParser(description='Run XTC synapse pipeline')
parser.add_argument('--input', nargs='+', dest='input_folders', help='One or more input directories that contain TIFF files')
parser.add_argument('--input-file', dest='input_file', help='A single input TIFF file')
parser.add_argument('--xy', type=float, help='XY resolution (um/pixel)')
parser.add_argument('--z', type=float, help='Z resolution (um/pixel)')
parser.add_argument('--output-dir', dest='output_dir', help='Directory where outputs should be written')
args, _ = parser.parse_known_args()

if args.input_folders is not None and args.input_file is not None:
    raise SystemExit('Use either --input or --input-file, not both.')

selected_input_file = None
output_root = os.path.abspath(args.output_dir) if args.output_dir is not None else None
if output_root is not None:
    os.makedirs(output_root, exist_ok=True)

if args.input_file is not None:
    if args.xy is None or args.z is None:
        raise SystemExit('When using --input-file, both --xy and --z are required.')
    selected_input_file = os.path.abspath(args.input_file)
    if not os.path.isfile(selected_input_file):
        raise SystemExit('Input file does not exist: ' + args.input_file)
    input_dir = os.path.dirname(selected_input_file)
    if not input_dir.endswith('/'):
        input_dir = input_dir + '/'
    list_folder = [input_dir]
    XY_res = float(args.xy)
    Z_res = float(args.z)
    print('Running in no-GUI mode with a single input file.')
elif args.input_folders is not None:
    if args.xy is None or args.z is None:
        raise SystemExit('When using --input, both --xy and --z are required.')
    list_folder = []
    for folder in args.input_folders:
        folder_abs = os.path.abspath(folder)
        if not os.path.isdir(folder_abs):
            raise SystemExit('Input directory does not exist: ' + folder)
        if not folder_abs.endswith('/'):
            folder_abs = folder_abs + '/'
        list_folder.append(folder_abs)
    XY_res = float(args.xy)
    Z_res = float(args.z)
    print('Running in no-GUI mode with CLI arguments.')
else:
...
for input_path in list_folder:    
    ...
    if output_root is not None:
        sav_dir = os.path.join(output_root, foldername + '_OUTPUT')
    else:
    ...
    if selected_input_file is not None:
        images = [selected_input_file]
    else:
        images = glob.glob(os.path.join(input_path,'*.tif'))
    images.sort(key=natsort_keygen(alg=ns.REAL))  # natural sorting
    
    all_adj_im = tiff.imread(images[0])
    # Support single-volume TIFF input (Z, Y, X) by adding a time axis.
    if all_adj_im.ndim == 3:
        all_adj_im = np.expand_dims(all_adj_im, axis=0)
    elif all_adj_im.ndim != 4:
        raise SystemExit('Input TIFF must be 3D (Z,Y,X) or 4D (T,Z,Y,X). Got shape: ' + str(all_adj_im.shape))
    ...
    if output_root is not None:
        enhance_out_dir = output_root + os.sep
    else:
        enhance_out_dir = '/cis/home/pwu60/Phoebe/xtc_copy/results/'
    os.makedirs(enhance_out_dir, exist_ok=True)
    ...
    xtc_output_path = enhance_out_dir + filename + '_REGISTERED_XTC_processed.tif'
    if selected_input_file is not None and all_XTC.shape[0] == 1:
        tiff.imwrite(xtc_output_path, np.asarray(all_XTC[0], dtype=np.uint8))
    else:
        ### must expand dimensions to save properly!
        tiff.imwrite(xtc_output_path, np.asarray(np.expand_dims(all_XTC, axis=2), dtype=np.uint8),
                     imagej=True, resolution=(10.5263157895, 10.5263157895),
    ...
    rescaled_output_path = enhance_out_dir + filename + '_REGISTERED_RESCALED_adj.tif'
    if selected_input_file is not None and all_reg.shape[0] == 1:
        tiff.imwrite(rescaled_output_path, np.asarray(all_reg[0], dtype=np.uint8))
    else:
        tiff.imwrite(rescaled_output_path, np.asarray(np.expand_dims(all_reg, axis=2), dtype=np.uint8),
                     imagej=True, resolution=(10.5263157895, 10.5263157895),
                     metadata={'spacing': 0.33, 'unit': 'um', 'axes': 'TZCYX'})    
    
    if os.environ.get('XTC_ONLY_ENHANCE', '0') == '1':
        print('XTC_ONLY_ENHANCE=1, skipping ILASTIK/MATLAB/tracking steps.')
        continue
    ...
    ************CANNOT HAVE ILASTIK BE RUNNING AT THE SAME TIME in GUI form!!! """    
    ...
    if output_root is not None:
        sav_dir_tmp = os.path.join(output_root, foldername + '_tmp_ILASTIK')
    else:
    ...
    for frame_id in range(len(all_XTC)):
        ...
        tiff.imwrite(tmp_name, np.asarray(input_im, dtype=np.uint8))
    ...
    skip_matlab = os.environ.get('XTC_SKIP_MATLAB', '0') == '1'
    if not skip_matlab and HAS_MATLAB_ENGINE:
        ...
        print('Skipping MATLAB watershed; using ILASTIK detections directly.')
        images_w = glob.glob(os.path.join(sav_dir_tmp, '*_REGISTERED_detect_ILASTIK.tif'))
